...
flowchart LR
    ...
    B -. same machinery .-> E[WM stop-weights sweep\n11 configs, 9 aborted\n2 winners that won by zeroing\nmost of their weights]
...
sequenceDiagram
    ...
    H->>P: POST /v1/query\n{plan_system_override: cand}
    P->>P: resolve override\n(body > file > builtin)
    P->>Pl: start query with override\nas SYSTEM message
    Pl->>R: hybrid_search / search_text
    R-->>Pl: hits
    Pl-->>P: final evidence packet
    P-->>H: {hits, run_id}
    H->>H: compute recall@k,\noverlap, lift vs ripgrep
    H->>H: backprop score\nup MCTS tree
```
...
```mermaid
flowchart TD
    P[planner emits\nstatus=stop, confidence c] --> Q{c above floor?}
    Q -->|no| R[reject: status=continue\nadd counter-hypothesis to failed_tail]
    Q -->|yes| C[adversarial critic call\ndifferent system prompt\nlooks for missing evidence]
    C --> D{critic confirms?}
    D -->|no| R
    D -->|yes| S[STOP: emit final hits\nterminate this stem]
    R --> N[next planner iteration\nsame node, updated context]
```
