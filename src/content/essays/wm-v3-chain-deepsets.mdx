...
flowchart LR
    subgraph trajectory["raw trajectory x_1 ... x_T"]
        X1["x_1\nsearch_text(auth_token)"]
        X2["x_2\nopen_file(login.py)"]
        X3["x_3\nsnippet_extract(L42-78)"]
        X4["x_4\nreferences_lookup(verify_jwt)"]
        X5["x_5\nhybrid_search(expired token)"]
    end
...
```mermaid
flowchart LR
    S["state_t\n(query, tool, args, snippet)"] --> E["codet5p-110m-embedding\n(frozen, 256-d)"]
    E --> G["GRU chain encoder\nper-step hidden h_i"]
    G --> PHI["per-step φ MLP\n256→512→512"]
    PHI --> POOL["DeepSets sum-pool\nz = Σ φ(x_i, h_i)"]
    POOL --> RHO["ρ readout MLP\n512→512"]
    RHO --> H1["value head\n51-bin HL-Gauss"]
    RHO --> H2["file_recall_t\nsigmoid"]
    RHO --> H3["snippet_recall_t\nsigmoid"]
    RHO --> H4["nano_prm\nregression"]
```
...
flowchart TB
    ...
    STORE --> RET
    RET --> BLEND["blend into UCB prior\nα · value + (1-α) · LLM_prior"]
```
