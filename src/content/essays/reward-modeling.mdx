...
sequenceDiagram
    ...
    Note over W,DB: row stays sampleable; failed parses\nget judge_source = harness_invocation_failed
```
...
flowchart TD
    ...
    B --> C[Readers don't read new columns:\nsilent struct drift]
    C --> D[Downstream computation appears to work\nshapes match, no nulls, no panics]
...
flowchart TD
    ...
    G --> I
    H --> I
    Z --> J[judge_label = NULL\nterminal_reward = 0]
    I --> K["R = 2·judge_label − 1"]
    K --> L["terminal credit:\nγ^(T−t) · R"]
    L --> M["v_tgt_t = discount sum\n+ terminal"]
    N[per-step shaping] --> O["+0.1·new_gold_hits\n− 0.05·empty_tool"]
    O --> P[r_t per step]
    P --> M
    M --> Q[value head:\nHL-Gauss 51-bin]
