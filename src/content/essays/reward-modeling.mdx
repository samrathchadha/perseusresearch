...
> at 39.59 USD / 60,565 rows, 82 minutes, was the per-step calibration
...
|-------------------|------------------------------|-------------------------|
| Cost / 60k rows   | 2 USD–4k GPU-hours (self-hosted 7B) | **39.59 USD** API cost |
...
| Re-cycle cost     | massive                      | 40 USD, weekly OK         |

**Cohort:** 60,565 rows at 39.59 USD (`training-pipeline.md` Step 5).
Concurrency 96, retry on 429, $\sim 12$ rps sustained throughput.

The earlier gpt-5.5-high estimate was 7,400 USD for the same row count
...
1. **Aspirational.** `gen_value_targets_v2.py` was scoped, the
   39.59 USD / 60,565 number is from a planned run that hasn't
   happened.
...
was scoped at 2 USD–4k of GPU-hours and never materialized because
three pieces of infrastructure were missing. nano-medium 16-dim is
scoped at 39.59 USD and the script that produces it does not exist in
...
- [nano judge distillation](/essays/nano-judge-distillation/) — the
  16-dim distillation plan, the 39.59 USD / 60,565-row cost ledger,
  the missing script.
