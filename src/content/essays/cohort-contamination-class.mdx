...
flowchart TD
    ...
    Silent --> A["Shape A — over-fanned key\n3,314 rows harness_collided"]
    Silent --> B["Shape B — NULL maps to 0.0\n17 days poisoned WM ckpts"]
    Silent --> C["Shape C — enum collapse\n786 rows invocation_failed"]
    Silent --> D["Shape D — row-split leakage\n8 days alpha=0.9 on memorization"]
...
```mermaid
flowchart LR
    A[New code change] --> B{introduces\nsilent default?}
    B -->|no| L[live]
    B -->|yes, caught at\ncode review| R1["cost: 1 hour\n(ADR-004 check)"]
    B -->|yes, lands| C{produces\nplausible value?}
    C -->|no| D1["cost: 1-3 days\n(downstream anomaly)"]
    C -->|yes| D2["cost: 10-20 days\n(detection latency)\n+ rework"]
    D2 --> E["compounding:\nWM ckpt redo,\ncohort filter, ADR write"]
```
...
flowchart TB
    subgraph PreventionRegime [Prevention regime — ADR-004 + ADR-005]
        P1["code-review gate\nC_prevent ~ 1 engineer-hour/change"]
        P2["type-system enforcement\ntyped errors, no unwrap_or"]
        P3["load-time refusal\nwm-serve rejects ckpt without metadata"]
        P1 --> P2 --> P3
    end
    subgraph DetectionRegime [Detection regime — audit after the fact]
        ...
        D2["trace causation\n~3-5 engineer-days"]
        D3["audit cohort\n~5-10 engineer-days"]
        D4["retrain downstream\n~7-17 calendar-days WM time"]
        D1 --> D2 --> D3 --> D4
    end
...
sequenceDiagram
    ...
    Note over Disk: split.strategy, verified_disjoint,\npolicy_fingerprint_shas, metrics
