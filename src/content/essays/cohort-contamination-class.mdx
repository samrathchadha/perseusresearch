...
graph TD
    ...
    Silent --> A[Shape A: over-fanned key\n3,314 rows in harness_collided]
    Silent --> B[Shape B: NULL → 0.0\n17 days of poisoned WM ckpts]
    Silent --> C[Shape C: enum collapse\n786 rows in harness_invocation_failed]
    Silent --> D[Shape D: row-split leakage\n8 days of α=0.9 trained on memorization]
    ...
    AFix --> Structural[Structural fix: ADR-004\nno-silent-fallback]
    BFix --> Structural
    CFix --> Structural
    DFix --> Provenance[Structural fix: ADR-005\ncheckpoint provenance]
```
