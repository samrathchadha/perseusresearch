...
```mermaid
flowchart LR
  Q[Q tile fp16] --> QKDot["Q·K^T dot\n(HMMA.884 fp16→fp16)"]
  K[K tile fp16] --> QKDot
  QKDot --> Widen1["Manual cast\nfp16 → f32"]
  Widen1 --> Acc1["f32 accumulator"]
  Acc1 --> Scale["·1/√d"]
  Scale --> Mask["causal + paged-block mask"]
  Mask --> Softmax["online softmax\n(running max + sum)"]
  Softmax --> SVDot["scores · V dot\n(HMMA.884 fp16→fp16)"]
  V[V tile fp16] --> SVDot
  SVDot --> Widen2["Manual cast\nfp16 → f32"]
...
sequenceDiagram
  ...
  C->>S: POST /chat/completions\nX-External-Session-Id: qry-abc123\nmessages=[SYSTEM, USER_v2]
  S->>S: tokenize → input_ids (3050 toks)
  S->>Cache: lookup(session_id=qry-abc123)
  Cache-->>S: cached_ids (3000 toks),\ncached_kv (DynamicCache)
  S->>S: longest_common_prefix(input_ids, cached_ids) = 3000
  S->>Cache: trim_to_length(3000)\n(drop nothing this call)
  S->>K: prefill(delta_ids[3000:3050], past_kv=cached_kv)
  K->>K: block-sparse attention\n(50 new queries vs 3000 cached keys)
  K-->>S: new kv blocks appended (3000→3050)
  S->>D: decode(input_ids[-1], past_kv)
  loop ≤ 256 new tokens
    D->>D: fused RMSNorm + qkv proj\n+ rotary + attention + SwiGLU
    D-->>S: next_token
